...
def _get_foreground_process() -> Optional[str]:
    ...
    try:
        ...
        # Fix: GetModuleFileNameExW requires PROCESS_QUERY_INFORMATION (0x0400)
        # AND PROCESS_VM_READ (0x0010). Using only 0x0400 causes the API to
        # return an empty path silently (no error, just an empty string result).
